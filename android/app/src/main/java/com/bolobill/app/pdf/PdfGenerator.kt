...
object PdfGenerator {
    ...
    fun getDynamicPhotoY(itemsCount: Int): Float {
        ...
        val calculatedY = baseTableY + tableHeight + 95f
    ...
    suspend fun generateInvoicePdf(context: Context, invoice: Invoice): File =
        withContext(Dispatchers.IO) {
            ...
            // Technician / Shop Details Header Box
            val techBox = RectF(PAGE_WIDTH - 230f, 22f, PAGE_WIDTH - 40f, 90f)
            ...
            canvas.drawText(invoice.technicianName.ifEmpty { "Service Provider" }, PAGE_WIDTH - 220f, 42f, techTitlePaint)
            if (invoice.technicianPhone.isNotEmpty()) {
                canvas.drawText("Phone: ${invoice.technicianPhone}", PAGE_WIDTH - 220f, 56f, metaPaint)
            } else {
                canvas.drawText(invoice.technicianTrade.ifEmpty { "General Work" }, PAGE_WIDTH - 220f, 56f, metaPaint)
            }
            if (invoice.technicianUpiId.isNotEmpty()) {
                canvas.drawText("UPI: ${invoice.technicianUpiId}", PAGE_WIDTH - 220f, 72f, metaPaint)
            }
            ...
            canvas.drawText(invoice.clientPhone, 52f, 145f, textPaint)
            ...
            // Summary (Subtotal, Advance, Balance)
            currentY += 10f
            val totalBoxX = PAGE_WIDTH - 240f
            canvas.drawText("कुल काम (Subtotal):", totalBoxX, currentY, textPaint)
            ...
            if (invoice.advanceAmount > 0) {
                currentY += 16f
                val advPaint = Paint().apply {
                    isAntiAlias = true
                    color = Color.rgb(2, 132, 199)
                    textSize = 10f
                }
                canvas.drawText("एडवांस मिला (Advance):", totalBoxX, currentY, advPaint)
                canvas.drawText("- ₹${String.format(Locale.ROOT, "%.2f", invoice.advanceAmount)}", totalBoxX + 110f, currentY, advPaint)
            }
            ...
            canvas.drawText("बाकी राशि (Payable):", totalBoxX, currentY, grandTotalPaint)
            ...
            // Work Proof Photos (Before & After)
            ...
            if (invoice.isPaid) {
                ...
                val upiTarget = invoice.technicianUpiId.ifEmpty { "payee@upi" }
                val upiUri = "upi://pay?pa=$upiTarget&pn=${URLEncoder.encode(invoice.technicianName, "UTF-8")}&am=${invoice.totalAmount}&cu=INR&tn=Bill_${invoice.invoiceNumber}"
    ...
    private fun drawPaidWatermark(canvas: Canvas) {
        ...
        canvas.drawText("भुगतान प्राप्त हुआ", rectLeft + 200f, rectTop + 52f, hindiWatermarkPaint)
        canvas.restore()
    }
